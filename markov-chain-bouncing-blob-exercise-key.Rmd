...
In this notebook we have a few exercises to do Monte Carlo sampling from our
...
ggplot(counts, aes(x = state, y = n, fill = factor(init))) +
  geom_col() +
  facet_wrap(~ init)
```
...
ggplot(counts10K, aes(x = state, y = n)) +
  geom_col()

# looking at that it appears identical to the distribution of states
# after 100 steps of many chains.
```
If you wanted to be more formal and plot these all together you could do:
```{r}
both <- counts10K %>%
  mutate(init = "time-average") %>%
  bind_rows(counts %>% mutate(init = as.character(init)), .) %>%
  group_by(init) %>%
  mutate(freq = n/sum(n))

ggplot(both, aes(x = state, y = freq, fill = init)) +
  geom_col() + 
  facet_wrap(~ init)
```
...
_It certainly appears so in that plot --- the time-average of the chain
looks to be the same as the long-run value average over multiple replicates._

5. How could you have known the answer without even simulating from
it?

_The chain is ergodic because it is irrducible, recurrent, and aperiodic, by inspection._
