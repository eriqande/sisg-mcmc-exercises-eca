...
M <- lapply(1:5, function(x)
  sim_tpm_multi(P = P, init = x, steps = 100, reps = 1000)
) %>%
  bind_rows()
...
How do they compare?
```{r}
long_run <- sim_tpm_multi(P = P, init = 2, steps = 10000, reps = 1)
...
We could have shown that the chain is recurrent, irreducible, and aperiodic.
